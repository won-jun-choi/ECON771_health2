...
reg_event <- feols(uncomp_care~i(relative_time, expanded, ref=-1) |provider+year,
                   cluster = ~state,
                   data = df_temp)

df_temp <- df_temp %>% filter((expanded==F)|year_adopted==2014)

reg_event_ss <- feols(uncomp_care~i(relative_time,expanded,ref=-1)|provider+year,
                      cluster = ~state,
                      data = df_temp)
```
```{r}

...
reg_DD_subsample <- function(ssyr) {
  df_temp <- df %>%
    filter(!is.na(uncomp_care)) %>%
    filter(expanded==FALSE | year_adopted == ssyr) %>%
    mutate(Y = uncomp_care/1000000,
           D = ifelse(year>=year_adopted,1,0),
           D = ifelse(!is.na(D),D,0))
...
df_temp <- df %>%
  filter(!is.na(uncomp_care)) %>%
  mutate(relative_time = year - as.integer(year_adopted),
         relative_time = ifelse(!is.na(relative_time),relative_time,0))

reg_event <- feols(uncomp_care~i(relative_time, expanded, ref=-1) | state+year,
                   cluster = ~state,
                   data = df_temp)

df_temp <- df %>%
  filter(!is.na(uncomp_care),
         (expanded==F)|year_adopted==2014 ) %>%
  mutate(relative_time = year - as.integer(year_adopted),
         relative_time = ifelse(!is.na(relative_time),relative_time,0))

reg_event_ss <- feols(uncomp_care~i(relative_time,expanded,ref=-1)|state+year,
                      cluster = ~state,
                      data = df_temp)
```

...
WAU <- function(name="wonjun",os) {
  ...
  print(paste0("Hi ",name,". You are on ",os))
}
WAU(name="wonjun",os="linux")
```

```{r Import data include=FALSE}
# Import data
df_HCRIS <- read.csv(paste0(dir_data,"/HCRIS-Ian/data/output/HCRIS_Data.txt"), sep = '\t', header=TRUE)
df_pos <- read.csv(paste0(dir_data,"/cms-pos-ian/data/output/pos-data-combined.txt"))
df_KFF <- read.csv(paste0(dir_data,"/Medicaid expansion from KFF/raw_data.csv"))
```
...
The related variables are:`tot_pat_rev`, `uncomp_care`, `tot_uncomp_care_charges`.

```{r}
df_HCRIS %>% 
  mutate(hosp_tot_rev = tot_pat_rev,
         uncomp_care = sum(tot_uncomp_care_charges,
                           uncomp_care, na.rm=T)) %>%
  group_by(year) %>% 
  summarise(mean=mean(tot_pat_rev,na.rm=T),
...
```{r}
df_HCRIS %>% filter(year==1998) %>% select(tot_uncomp_care_charges)
```
